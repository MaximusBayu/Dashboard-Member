      ...
      <form onSubmit={handleSubmit} className="grid grid-cols-2 gap-5">
        ...
        <div className="mb-5 col-span-2">
          ...
          <input
            ...
            className="w-full p-2 text-lg border"
          />
          {memberData.photo && <img src={URL.createObjectURL(memberData.photo)} alt="Preview" className="mt-2 w-32 h-32 object-cover" />}
        </div>
        <div className="mb-5 col-span-2 flex items-center">
          <label className="block font-bold mb-2 mr-3">Status Member:</label>
          <span className="text-green-500">Aktif</span>
        </div>
        <div className="mb-5 col-span-2 flex justify-between">
          <button type="button" className="px-5 py-3 text-lg bg-gray-200 text-black border border-gray-400 cursor-pointer">Edit</button>
          <button type="submit" className="px-5 py-3 text-lg bg-blue-500 text-white border-none cursor-pointer">Simpan</button>
        </div>
      </form>
    </div>
  );
